...
1. **Read state**:
   ```bash
   REVIEW_FIXES=$(.agents/skills/linear-orch/scripts/workflow-state get [ISSUE_ID] '.fixed_items // []')
   PR_FIXES=$(.agents/skills/linear-orch/scripts/workflow-state get [ISSUE_ID] '.pr_comment_review.fixes // []')
   ```
